...
class SemiLepBDecayTagger(Module):
    ...
    def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
        ...
        self.out.branch("ToMaAn_GenSoftMu_ptOverBHadPt", "F")
        ...
        self.histos['DR_softMu_associatedJet'] = ROOT.TH1F( 'DR_softMu_associatedJet', ';#Delta R(#mu_soft,#jet); Soft Muons',100,0.,1.)
        ...
        self.histos['matchedSoftMu_ptOverJetPt'] = ROOT.TH1F( 'matchedSoftMu_ptOverJetPt', ';pt_matchedSoftMu/pt_associatedJet; soft and matched Muons',100,0,1)
        ...
        self.histos[ 'MlMu_MCTruth_sameTop' ] = ROOT.TH1F( 'MlMu_MCTruth_sameTop', ';MCTruth M_{#mu_{soft},#mu_{hard}}; Events',250,0.,250.)
        self.histos[ 'MlMu_MCTruth_oppositeTop' ] = ROOT.TH1F( 'MlMu_MCTruth_oppositeTop', ';MCTruth M_{#mu_{soft},#mu_{hard}}; Events',250,0.,250.)
    ...
    def analyze(self, event):
        ...
        self.out.fillBranch( 'ToMaAn_GenSoftMu_ptOverBHadPt' , -1. )
	...
	p4_bs = []
        ...
        for p in particles:
            ...
            if abs(p.pdgId) in [13]: # electrons removed from the list because we are only interested in semi-muonic B decays
              ...
              if bHadronProduct:
                  ...
                  if abs(gmothpid) not in [1,2,3,4,21]:
                   ...
                   if abs(gmothpid) not in [5]:
                    if abs(ggmothpid) not in [1,2,3,4,21]: # to account for the excited b-hadrons as ggmother particles
                       nrSemiMuBs+=1
                       self.out.fillBranch("ToMaAn_GenSoftMu_pt", p.pt)
                       self.out.fillBranch("ToMaAn_GenSoftMu_ptOverBHadPt", p.pt/moth.pt)
                       genSoftMus.append(p)
                       # try to find the pdgid of the mother top quark
                       while abs(ggmothpid) != 6 and ggmothidx > 0 :
                        ggmothidx  = ggmoth.genPartIdxMother
                        if ggmothidx >= 0 :
                         ggmoth    = particles[ggmothidx]
                         ggmothpid = ggmoth.pdgId
                       genSoftMu_topPID = ggmothpid
                   else:
                       nrSemiMuBs+=1
                       self.out.fillBranch("ToMaAn_GenSoftMu_pt", p.pt)
                       self.out.fillBranch("ToMaAn_GenSoftMu_ptOverBHadPt", p.pt/moth.pt)
              ...
              # try to find the pdgid of the mother top quark
               ...
               #  moth    = particles[mothidx]
               #  mothpid = moth.pdgId
               #genHardMu_topPID = mothpid
        #if genSoftMu_topPID != 0 : print("          --- top quark pid for gensoftmu : ",genSoftMu_topPID)
        #if genHardMu_topPID != 0 : print("          --- top quark pid for genhardmu : ",genHardMu_topPID)
        ...
        if len(genSoftMus) == 1 and len(genMus) == 1 :
        #if len(genSoftMus) > 0 and len(genMus) == 1 :  # 25 Feb
            mcTruthMass = (genSoftMus[0].p4()+genMus[0].p4()).M()
            self.out.fillBranch("ToMaAn_MlMu_MCTruth", mcTruthMass)
            if genHardMu_topPID*genSoftMu_topPID == 36:
                self.histos[ 'MlMu_MCTruth_sameTop' ].Fill( mcTruthMass )
            if genHardMu_topPID*genSoftMu_topPID == -36:
                self.histos[ 'MlMu_MCTruth_oppositeTop' ].Fill( mcTruthMass )
        ...
        for m in muons:
            ...
            # id cut
            if not m.looseId: continue
            #if not m.mediumId: continue
            #if not m.softId: continue
            #Loose transverse and longitudinal impact parameter cuts, dxy < 0.3 cm and dz < 20 cm w.r.t. the primary vertex
            if not m.dxy < 0.3: continue
            if not m.dz < 20: continue
            if not m.nTrackerLayers > 5: continue
            ...
            if m.jetIdx in good_bjets_indices and soft_m_index not in good_muons_indices:
                for bj in range(len(good_bjets_indices)):
                 if m.jetIdx != good_bjets_indices[bj]: continue   
                 self.histos[ 'DR_softMu_associatedJet' ].Fill( jets[m.jetIdx].DeltaR(m) )
                 ...
                 #print("         whether identified as softMvaId? ",m.softId)
                 
                 if nrSemiMuBs == 1 and m.DeltaR(genSoftMus[0]) < 0.3:
                    self.histos[ 'matchedSoftMu_ptOverJetPt' ].Fill( m.pt/jets[m.jetIdx].pt )
                        
                 if m.pt/jets[m.jetIdx].pt > 0.3:   
                     #if m.pt>50:
                      #  print("************************  soft reconstructed muon found !!")
                      #  print("         , and pt: ",good_muons[0].pt,"  , and eta: ",m.pt)
        ...
        if nrSemiMuBs == 1:
        #if nrSemiMuBs > 0: # 25 Feb
            self.nrSemiMuBDecays += 1
        soft_mu_mother = 0.
        ObservableMass = -1.
        if len(good_soft_muons) == 1:
        #if len(good_soft_muons) > 0: # changed on 25 Feb, to account for only events with exactly one semi-mu b-decays
            self.nrSoftRecMu += 1
        ...
        if not len(good_soft_muons) == 1: 
        #if len(good_soft_muons) == 0: # 25 Feb
            return False
              
        self.histos[ 'softRecMu_pt' ].Fill( good_soft_muons[0].pt )

        if len(genSoftMus) == 1:
        #if len(genSoftMus) > 0: # 25 Feb
        ...
        if good_soft_muons[0].DeltaR(good_muons[0]) < 2. : 
            ...
            if len(genSoftMus) == 1 and good_soft_muons[0].DeltaR(genSoftMus[0]) < 0.3:
            #if len(genSoftMus) > 0 and good_soft_muons[0].DeltaR(genSoftMus[0]) < 0.3: # 25 Feb
